...
public:
  ...
  Polynomial operator+=(const Polynomial& p) {
    for (int i = 0; i < p.size(); ++i) {
      if (int(this->size()) == i) this->emplace_back(p[i]);
      else (*this)[i] += p[i];
    }
    normalize();
    return *this;
  ...
  Polynomial operator*=(const Polynomial& p) {
    ...
    if (min(a, b) <= 256) {
      ...
      for (int i = 0; i < a; ++i) {
        for (int j = 0; j < b; ++j) res[i + j] += (*this)[i] * p[j];
      }
      *this = res;
      normalize();
      return *this;
  ...
  Polynomial operator/=(const Polynomial& p) {
    ...
    for (int i = int(this->size()) - int(p.size()); i >= 0; --i) {
      ...
      for (int j = 0; j < p.size(); ++j) (*this)[i + j] -= res[i] * p[j];
    }
    *this = res;
    normalize();
    return *this;
  ...
  Polynomial operator%=(const Polynomial& p) {
    for (int i = int(this->size()) - int(p.size()); i >= 0; --i) {
      ...
      for (int j = 0; j < p.size(); ++j) (*this)[i + j] -= d * p[j];
    }
    normalize();
    return *this;
  }

  static constexpr Polynomial identity() {
