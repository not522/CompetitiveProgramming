...
public:
  ...
  void maximal(Vector<Vector<int>> &cliques, Vector<int> &current,
               Vector<int> remains) {
    if (remains.empty()) {
      for (int i = 0; i < graph.size(); ++i) {
        ...
        if (none_of(current.begin(), current.end(), [this, i](int v) {
              return graph.getEdges(i, v).empty();
            })) {
          return;
        }
      }
    ...
    for (int v : remains) {
      ...
      Vector<int> next;
      for (int vv : remains) {
        if (!graph.getEdge(v, vv).isNone()) {
          next.emplace_back(vv);
        }
      }
  ...
  void maximum(Vector<int> &maximumClique, Vector<int> &current,
               Vector<int> remains) {
    if (maximumClique.size() < current.size()) {
      maximumClique = current;
    }
    int maxSize = maximumClique.size();
    if (int(current.size() + remains.size()) <= maxSize) {
      return;
    }
    if (graph.getDegree(remains[maxSize - current.size()]) < maxSize) {
      return;
    }
    for (int v : remains) {
      current.emplace_back(v);
