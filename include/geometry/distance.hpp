...
Real distance(const Segment &segment, const Point &point) {
  ...
  if (p.x < 0) {
    return (p * segment.vec()).abs();
  }
  if (p.x > 1) {
    return ((p - Point(1, 0)) * segment.vec()).abs();
  }
...
Real distance(const Segment &segment1, const Segment &segment2) {
  ...
  return min(
      min(distance(segment1, segment2.a), distance(segment1, segment2.b)),
      min(distance(segment2, segment1.a), distance(segment2, segment1.b)));
}
