...
private:
  struct RangeValue {
    int i, j, y, x;
    Value v;
    RangeValue(int i, int j, int y, int x, Value v)
        : i(i), j(j), y(y), x(x), v(v) {}
  };
...
public:
  ...
  void add(int i, int j, Value v) { add(i, j, i + 1, j + 1, v); }
  ...
  Value value(int i, int j) { return sum(i, j, i + 1, j + 1); }
};
