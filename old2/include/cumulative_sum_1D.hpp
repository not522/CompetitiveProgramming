...
public:
  ...
  void add(int i, Value v) { add(i, i + 1, v); }

  void add(int i, int j, Value v) { rangeValue.emplace_back(i, j, v); }
  ...
  Value value(int i) { return sum(i, i + 1); }

  Value max() {
    auto res = std::numeric_limits<Value>::lowest();
